...
#include <unistd.h> //Librería que contiene funciones para el manejo de directorios y archivos
	...
	while(true){
		for(int i = 0; i < 24; i++){
			if(i == 3 || i == 7 || i == 11 || i == 15 || i == 19 || i == 23){ //Para que el circuito cierre
				...
				cout << "El tiempo para conducir terminó" << endl;
				Sleep(1000);
...
void verificar_kart(){
	...
	if(num_random==1){
		system("COLOR 70");
		cout<<"Ocurrió un accidente, esperando al mecanico"<< endl;
		semaforo.wait();
		...
		if(num_random == 0){
			...
			semaforo.wait();
			cout<<"Terminó el mantenimiento"<< endl;
			system("COLOR 07" );
		}
	}
	if(num_random==2){
		system("COLOR 70" );
		cout<<"Ocurrió una falla mecanica en un kart, esperando al mecanico"<< endl;
		semaforo.wait();
		cout<< endl;
		cout<<"El mecanico " << mecanico << " a revisado el kart" <<endl;
